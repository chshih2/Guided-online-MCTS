...
class GraphVisualisation():
    ...
    def game_node(self, graph, game, node, visited, level, print_state, print_value):
        if node.id not in visited:
            graph.node(str(node.id), label=self.node_to_string(game, node, print_state, print_value),
                       xlabel=str(node.player_turn) if node.player_turn is not None else "")
            if level <= self.max_level:
