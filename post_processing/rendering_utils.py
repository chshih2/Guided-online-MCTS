...
def render_action_q_value(tileSize, x, y, action, q_values, img, show_text=False, text_size=12, h_text_offset=0,
                          v_text_offset=0, rewardMax=1, rewardMin=1):
    value = q_values.get_q_value((x, y), action)  # MDP.get_q_value(q_values, (x, y), action=action)
    colour = COLOURS['red'] if value < 0 else COLOURS[
        'green']  # make colour red if value is negative, otherwise make it green
    ...
    if show_text:
        plt.text(x=x * tileSize + tileSize // 2 + h_text_offset, y=y * tileSize + tileSize // 2 + v_text_offset,
                 s=f'{value:.2f}', size=text_size, verticalalignment='center', horizontalalignment='center',
                 color='white')
...
def render_action_probability(tileSize, x, y, action, prob, text_size=6, h_text_offset=0, v_text_offset=0):
    plt.text(x=x * tileSize + tileSize // 2 + h_text_offset, y=y * tileSize + tileSize // 2 + v_text_offset,
             s=f'{prob:.2f}\n{action}', size=text_size, verticalalignment='center', horizontalalignment='center',
             color='white')
